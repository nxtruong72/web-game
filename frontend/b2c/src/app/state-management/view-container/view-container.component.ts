import { Component, ChangeDetectionStrategy, Input, TemplateRef, AfterViewInit, ViewContainerRef } from '@angular/core';
import { View } from '../interface/view.interface';
...
export class ViewContainerComponent<T> implements AfterViewInit {
  private _mainTemplateRef!: TemplateRef<AppViewContext<T>>;
  private _errorTemplateRef!: TemplateRef<AppViewContext<T>>;
  private _loaderTemplateRef!: TemplateRef<AppViewContext<T>>;
  ...
  @Input() set view(view: View<T>) {
    ...
    if (view.loader) this._viewContainer.createEmbeddedView(this._loaderTemplateRef, this._context);

    if (view.error && !view.loader) this._viewContainer.createEmbeddedView(this._errorTemplateRef, this._context);

    if (view.data && !view.error) this._viewContainer.createEmbeddedView(this._mainTemplateRef, this._context);
  }
